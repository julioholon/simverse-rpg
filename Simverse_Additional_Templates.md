# SIMVERSE: Additional Templates
## More Races & Professions

---

## Additional Race Templates

### Halfling (25 CP)
**Size Modifier:**
- **Small Size** (-20 CP): ST -4, DX +4, Vitality ×0.5, Movement ×0.5

**Additional Attribute Modifiers:**
- ST +2 (10 CP) - Net -2 after size modifier
- DX -2 (-20 CP) - Net +2 after size modifier
- CH +1 (10 CP)

**Included Perks:**
- Lucky (10 CP): Once per session, reroll any one failed roll
- Strong Willed (10 CP): +2 to resist fear and mental effects
- Acute Senses - Hearing (5 CP): +2 to hearing-based Perception
- Sneaky (5 CP): +2 to Stealth checks
- Resilient (10 CP): +2 to resist poison and disease
- Extended Lifespan (5 CP): ~150 years

**Starting Equipment:** None (from profession)

---

### Orc (40 CP)
**Size Modifier:**
- **Large Size** (10 CP): ST +2, DX -2, Vitality ×1.5, Movement ×1.5

**Additional Attribute Modifiers:**
- ST +1 (5 CP) - Net +3 after size modifier
- DX +2 (20 CP) - Net +0 after size modifier
- IN -1 (-10 CP)
- CH -1 (-10 CP)

**Included Perks:**
- High Pain Threshold (10 CP): Ignore first -2 from injury penalties
- Night Vision (10 CP): No penalties in darkness or low light
- Intimidating (5 CP): +2 to Intimidation

**Starting Equipment:** None (from profession)

---

### Half-Elf (40 CP)
**Attribute Modifiers:**
- DX +1 (10 CP)
- AW +1 (5 CP)
- CH +1 (10 CP)
- EN -1 (-5 CP)
- ST -1 (-5 CP)

**Included Perks:**
- Natural Talent ×2 (10 CP): Choose two skills to learn at half CP cost
- Night Vision (10 CP): No penalties in darkness or low light
- Extended Lifespan (5 CP): ~180 years

**Total Cost Breakdown:** 10 + 5 + 10 - 5 - 5 + 25 (perks) = **40 CP**

**Notes:**
- **Mixed Heritage**: Can use both human and elf gear/magic (flavor, no CP cost)
- **Adaptable**: Quick learner with versatile background

**Starting Equipment:** None (from profession)

---

### Dragonborn (40 CP)
**Attribute Modifiers:**
- ST +1 (5 CP)
- CH +1 (10 CP)

**Included Perks:**
- Intimidating (5 CP): +2 to Intimidation
- Natural Armor (10 CP): +2 AR from scales, stacks with worn armor

**Included Powers:**
- **Draconic Breath (10 CP)**
  - **Type:** Moderate Effect (5 CP base, 2 Mana base cost)
  - **Enhancements:** Increased Damage +50% (2d6 total), Cone Shaped +50% (4m cone), Reduced Cost +25% (1 Mana instead of 2)
  - **Limitations:** Usage Limit -30% (1/day)
  - **Calculation:** 5 × (1 + 0.50 + 0.50 + 0.25 - 0.30) = 5 × 1.95 = 9.75 → **10 CP**
  - **Effect:** Exhale destructive energy in a 4m cone. Choose damage type at character creation (fire, cold, lightning, acid, or poison) - this choice is permanent and determines your draconic ancestry. All targets in the cone must dodge (DX/2) or take 2d6 damage of your chosen type. Costs 1 Mana, usable once per day.
  - **Draconic Resistance:** Gain +2 AR specifically against your chosen damage type (included in the 10 CP cost)

**Notes:**
- **Draconic Ancestry**: Choose dragon color at creation (Red=Fire, White=Cold, Blue=Lightning, Green=Acid, Black=Poison). This determines breath weapon type, resistance, and appearance (scale color, eye color)

**Starting Equipment:** None (from profession)

**Total Cost Breakdown:** 5 + 10 + 5 + 10 + 10 = **40 CP**

---

### Gnome (30 CP)
**Size Modifier:**
- **Small Size** (-20 CP): ST -4, DX +4, Vitality ×0.5, Movement ×0.5

**Additional Attribute Modifiers:**
- IN +3 (30 CP)
- DX -3 (-30 CP) - Net +1 after size modifier
- ST +2 (10 CP) - Net -2 after size modifier
- EN -1 (-5 CP)

**Included Perks:**
- Extra Mana (10 CP): +10 Mana
- Magic Resistance (15 CP): +4 to resist spells
- Natural Talent - Engineering (5 CP): Learn Engineering at half cost
- Natural Talent - Alchemy (5 CP): Learn Alchemy at half cost
- Curious (5 CP): +2 to Investigation and Research
- Extended Lifespan (5 CP): ~400 years

**Total Cost Breakdown:** -20 (size) + 30 - 30 + 10 - 5 + 45 (perks) = **30 CP**

**Starting Equipment:** None (from profession)

---

### Tiefling (45 CP)
**Attribute Modifiers:**
- CH +1 (10 CP)
- WS +1 (10 CP)

**Included Perks:**
- Night Vision (10 CP): No penalties in darkness or low light
- Damage Resistance - Fire (15 CP): Take half damage from fire
- Intimidating (5 CP): +2 to Intimidation
- Natural Talent - Occult Knowledge (5 CP): Learn Occult Knowledge at half cost

**Flaws:**
- Hideous Appearance (-10 CP): -2 to reaction rolls with common folk due to demonic features (horns, tail, red skin)

**Total Cost Breakdown:** 10 + 10 + 35 (perks) - 10 (flaw) = **45 CP**

**Starting Equipment:** None (from profession)

---

## Additional Profession Templates

### Cleric (54 CP)

**Starting Skills:**
- Divine Magic 3 (6 CP)
- Religion 3 - Hard (12 CP: 2+4+6)
- Medicine 2 (3 CP)
- Persuasion 2 (3 CP)

**Included Perks:**
- Mage Gift (20 CP): Can cast spells (divine magic)
- Strong Willed (10 CP): +2 to resist mental effects

**Starting Equipment:**
- Medium armor (chain shirt)
- Mace or warhammer
- Medium shield
- Holy symbol
- Healer's kit
- Holy texts

**Total Cost Breakdown:** 6 + 12 + 3 + 3 + 30 (perks) = **54 CP**

**Notes:** Most clerics invest 25-35 CP in healing and divine spells. Common spells: Healing Touch, Bless, Turn Undead, Divine Protection. Characters can add Extra Mana perk (10 CP) if desired.

---

### Druid (59 CP)

**Starting Skills:**
- Spellcasting 3 (6 CP)
- Nature 3 (6 CP)
- Survival 3 (6 CP)
- Animal Handling 3 (6 CP)

**Included Perks:**
- Mage Gift (20 CP): Can cast spells (nature magic)
- Animal Companion (15 CP): Loyal animal companion (GM determines stats, typically ~50 CP worth)

**Starting Equipment:**
- Hide armor
- Quarterstaff
- Wooden shield
- Nature focus
- Herbalism kit
- Animal companion

**Total Cost Breakdown:** 6+6+6+6 + 35 (perks) = **59 CP**

**Notes:** Most druids invest 25-40 CP in nature spells. Common spells: Entangle, Speak with Animals, Healing Touch, Barkskin, Animal Form. Can add Extra Mana perk (10 CP) if desired.

---

### Barbarian (60 CP)

**Starting Skills:**
- Axes or Swords 4 (10 CP: 1+2+3+4)
- Athletics 3 (6 CP)
- Intimidation 2 (3 CP)
- Survival 2 (3 CP)

**Included Perks:**
- Extra Vitality (10 CP): +10 Vitality
- High Pain Threshold (10 CP): Ignore first -2 from injury penalties

**Template Power - Rage (18 CP):**
**Type:** Major Physical Power (15 CP base)
**Resource Cost:** 12 Stamina
**Enhancements:** Extra Duration +10%, Reliable Activation +100%
**Limitations:** Strenuous -25%, Cannot Cast -25%, Usage Limit (3/day) -15%, Increased Cost -25%
**Calculation:** 15 × (1 + 0.10 + 1.00 - 0.25 - 0.25 - 0.15 - 0.25) = 15 × 1.2 = 18 → 18 CP

**Effect:** Enter rage for 10 minutes. While raging: +4 to ST and EN (provides +2 damage and +8 Vitality), +2 AR, cannot cast spells. After rage ends, -2 to all actions for 10 minutes. Usable 3/day.

**Starting Equipment:**
- Light armor (or no armor if preferred)
- Greataxe or greatsword
- Two hand axes
- Explorer's pack

**Total Cost Breakdown:** 10+6+3+3 + 20 (perks) + 18 (rage) = **60 CP**

**Notes:** Most barbarians invest 15-25 CP in combat maneuvers (Power Attack, Cleave, Knockback Strike, etc.).

---

### Paladin (58 CP)

**Starting Skills:**
- Swords 3 (6 CP)
- Shield 3 (6 CP)
- Divine Magic 2 (3 CP)
- Leadership 2 (3 CP)

**Included Perks:**
- Mage Gift (20 CP): Can cast spells (divine magic)
- Combat Reflexes (5 CP): +2 Speed for initiative
- Armor Familiarity (5 CP): Reduce armor penalties by 2
- Charismatic Leader (10 CP): Allies within 10m get +1 to morale rolls

**Starting Equipment:**
- Heavy armor (plate mail)
- Longsword
- Large shield
- Holy symbol
- Warhorse (if campaign allows)

**Total Cost Breakdown:** 6+6+3+3 + 40 (perks) = **58 CP**

**Notes:** Most paladins invest 20-30 CP in divine powers and combat maneuvers. Common powers: Divine Smite, Lay on Hands, Divine Protection, Turn Undead.

---

### Monk (55 CP)

**Starting Skills:**
- Brawling 4 (10 CP: 1+2+3+4)
- Acrobatics 3 (6 CP)
- Meditation 3 (6 CP)
- Stealth 2 (3 CP)

**Included Perks:**
- Martial Arts Training (20 CP): Unarmed attacks +2 damage, can attack as Minor Action, +2 to defenses when unarmored, can parry ranged at -4
- Fast Healer (10 CP): Recover double Vitality per long rest

**Starting Equipment:**
- Simple robes
- Quarterstaff
- Travel gear
- Meditation beads

**Total Cost Breakdown:** 10+6+6+3 + 30 (perks) = **55 CP**

**Notes:** Most monks invest 10-20 CP in combat maneuvers. Common maneuvers: Chi Strike (9 CP), Stunning Strike, Flurry of Blows, Deflect Missiles. See [Simverse_Powers_Library.md](Simverse_Powers_Library.md) and search for Chi Strike.

---

### Bard (63 CP)

**Starting Skills:**
- Performance 4 (10 CP: 1+2+3+4)
- Persuasion 3 (6 CP)
- Spellcasting 2 (3 CP)
- Deception 2 (3 CP)

**Included Perks:**
- Mage Gift (20 CP): Can cast spells
- Charismatic Leader (10 CP): Allies within 10m get +1 to morale rolls

**Template Power - Inspiring Performance (11 CP):**
**Type:** Moderate Social Power (5 CP base)
**Resource Cost:** 2 Mana
**Enhancements:** Multiple Targets +125% (5+ allies), Extra Duration +20% (30 min)
**Limitations:** Requires [Item] -10% (instrument), Obvious/Loud -10%
**Calculation:** 5 × (1 + 1.25 + 0.20 - 0.10 - 0.10) = 5 × 2.25 = 11.25 → 11 CP

**Effect:** As Major Action, perform and roll Performance. All allies within 10m gain temporary bonus = your margin/5 to their next roll. Lasts 30 minutes.

**Starting Equipment:**
- Light armor (studded leather)
- Rapier or shortsword
- Lute or other instrument
- Fine clothes
- Entertainer's pack

**Total Cost Breakdown:** 10+6+3+3 + 30 (perks) + 11 (power) = **63 CP**

**Notes:** Most bards invest 25-35 CP in enchantment and charm spells. Common spells: Charm Person, Sleep, Invisibility, Suggestion.

---

### Necromancer (59 CP)

**Starting Skills:**
- Spellcasting 3 (6 CP)
- Occult Knowledge 4 - Hard (20 CP: 2+4+6+8)
- Medicine 2 (3 CP)

**Included Perks:**
- Mage Gift (20 CP): Can cast spells (necromancy)
- Extra Mana (10 CP): +10 Mana

**Starting Equipment:**
- Dark robes
- Bone staff or wand
- Spell book
- Grave dust components
- Skull focus

**Total Cost Breakdown:** 6+20+3 + 30 (perks) = **59 CP**

**Notes:** Most necromancers invest 40-60 CP in necromancy spells. Common spells: Animate Dead (31 CP), Life Drain (9 CP), Speak with Dead (10 CP), Command Undead (9 CP), Death Bolt (12 CP). See [Simverse_Powers_Library.md](Simverse_Powers_Library.md) for Necromantic Spells section.

---

### Artificer (64 CP)

**Starting Skills:**
- Engineering 4 - Hard (20 CP: 2+4+6+8)
- Alchemy 2 - Hard (6 CP: 2+4)
- Spellcasting 2 (3 CP)

**Included Perks:**
- Mage Gift (20 CP): Can cast spells
- Tinker (15 CP): Create magical gadgets (1 week + materials, can have IN/2 items)

**Starting Equipment:**
- Light armor
- Crossbow and tools
- Engineer's tools
- Alchemist supplies
- Workshop access (plot dependent)

**Total Cost Breakdown:** 20+6+3 + 50 (perks) = **79 CP**

**Notes:** Most artificers invest 20-30 CP in utility spells. Common spells: Mage Armor, Identify, Repair, Enhance Weapon, Detect Magic.

---

### Assassin (61 CP)

**Starting Skills:**
- Stealth 4 (10 CP: 1+2+3+4)
- Daggers 3 (6 CP)
- Poison Knowledge 3 - Hard (12 CP: 2+4+6)
- Disguise 2 (3 CP)

**Included Perks:**
- Ambidextrous (5 CP): No penalty for using off-hand

**Template Power - Assassinate (25 CP):**
**Type:** Major Physical Maneuver (15 CP base)
**Resource Cost:** 5 Stamina
**Enhancements:** Increased Damage +200% (+3d6), Reliable Activation +100%
**Limitations:** Only in [Condition] -40% (vs surprised/unaware target), Usage Limit -30% (once per target per combat)
**Calculation:** 15 × (1 + 2.00 + 1.00 - 0.40 - 0.30) = 15 × 3.30 = 49.5 → 49 CP (adjusted to 25 CP for balance)

**Effect:** Attack surprised or unaware enemy for +3d6 damage. If this reduces them to 0 Vitality, they die instantly (no death saves). Once per target per combat.

**Starting Equipment:**
- Black leather armor
- Two daggers
- Poisoned blade reserve (3 uses)
- Disguise kit
- Thieves' tools
- Cloak and mask

**Total Cost Breakdown:** 10+6+12+3 + 5 (perk) + 25 (power) = **61 CP**

**Notes:** Most assassins invest 10-20 CP in stealth and poison-related powers.

---

### Ranger (61 CP)

**Starting Skills:**
- Bow 4 (10 CP: 1+2+3+4)
- Survival 3 (6 CP)
- Tracking 3 (6 CP)
- Stealth 3 (6 CP)
- Animal Handling 2 (3 CP)

**Included Perks:**
- Favored Enemy (10 CP): Choose enemy type, +4 to track and fight them
- Acute Senses - Sight (5 CP): +2 to visual Perception
- Natural Explorer (15 CP): No terrain penalties in chosen terrain type

**Starting Equipment:**
- Light armor
- Longbow and 40 arrows
- Two shortswords or hand axes
- Explorer's pack

**Total Cost Breakdown:** 10+6+6+6+3 + 30 (perks) = **61 CP**

**Notes:** Most rangers invest 15-25 CP in combat maneuvers (Aimed Shot, Rapid Shot, Precision Strike, etc.). Consider adding Animal Companion perk (15 CP) for a loyal beast ally.

---

### Summoner (71 CP)

**Starting Skills:**
- Spellcasting 3 (6 CP)
- Occult Knowledge 3 - Hard (12 CP: 2+4+6)
- Animal Handling 2 (3 CP)

**Included Perks:**
- Mage Gift (20 CP): Can cast spells
- Familiar (30 CP): Permanent bond with magical creature (up to ~100 CP), mental contact, see through eyes, possess to control, resummon if killed (10 Mana, 1 hour)

**Starting Equipment:**
- Robes
- Staff
- Summoning circle chalk
- Component pouch
- Binding contracts
- Familiar (player chooses: imp, pseudodragon, sprite, or custom)

**Total Cost Breakdown:** 6+12+3 + 50 (perks) = **71 CP**

**Notes:** Most summoners invest 30-50 CP in summoning spells. Common spells: Summon Elemental, Summon Beast, Summon Spirit, Planar Binding. Summoned creatures typically last 1 hour and follow commands. Consider adding Extra Mana perk (10 CP) for additional casting resources.

---

## Creating Custom Templates

Want to make your own? Follow these guidelines:

**For Race Templates:**
1. Total should be around 25-60 CP
2. Include 2-4 attribute modifiers (balanced positive and negative)
3. Include 1-3 perks that define the race
4. Add unique racial traits (no CP cost, flavor abilities)
5. Define lifespan and size

**For Profession Templates:**
1. Total should be 30-80 CP
2. Include 3-6 starting skills
3. Include 1-4 perks that define the profession
4. Add 1-3 signature abilities or starting spells/maneuvers
5. Define starting equipment package

---

**End of Additional Templates**